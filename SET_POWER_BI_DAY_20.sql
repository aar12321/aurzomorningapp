-- Set Power BI Basics to day 20 for rohanchax@gmail.com

DO $$
DECLARE
  user_record_id UUID;
  power_bi_topic_id UUID;
  target_day INTEGER := 20;
  max_day INTEGER;
BEGIN
  -- Get the user ID
  ...
  -- Get max day for Power BI Basics (cap at target_day)
  SELECT LEAST(MAX(day_number), target_day) INTO max_day
  FROM public.quizzes
  ...
  -- Ensure we don't exceed target_day
  IF max_day > target_day THEN
    max_day := target_day;
  END IF;
